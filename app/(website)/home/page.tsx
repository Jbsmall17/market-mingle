...
export default function Home() {
  const Product = ({
    ...
    return (
      <div
        className={`w-[225px] h-[250px] relative rounded-lg bg-white ${
          type === "second" ? "border border-[#62a643]" : "border-none"
        }`}
      >
        <div
          className={`h-[62.5%] ${
            type === "first" ? "bg-[#a6cf92]" : "bg-white"
        ...
        >
          ...
          <Image src={imgPath} alt={`${name}'s image`} width={60} height={80} />
        </div>
        <div className="px-2 h-auto">
          ...
          <p className="flex gap-1">
            {ratingArr.map((fig, idx) => {
              if (fig <= rating) {
                return <Star className="size-4" key={idx} />;
